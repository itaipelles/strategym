...
if __name__ == "__main__":
    ...
    model_russia = PPO('MultiInputPolicy', env, verbose=4, n_steps=512*2, batch_size=512, policy_kwargs=policy_kwargs, device='cpu')
    model_germany = PPO('MultiInputPolicy', env, verbose=4, n_steps=512*2, batch_size=512, policy_kwargs=policy_kwargs, device='cpu')
    ...
    for generation_num in range(num_of_generations):
        for player in Players:
            ...
            logger = configure(f'./logger/{player.name}_{generation_num}', ["stdout", "log", "csv"])
            policies[player].set_logger(logger)
